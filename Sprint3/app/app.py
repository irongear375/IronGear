    ...
    for box in text_boxes:
        ...
        try:
            ...
            for text in results:
                ...
                if text_upper.startswith("L") and laterality == "Unknown":
                    laterality = "Left"
                elif text_upper.startswith("R") and laterality == "Unknown":
                    laterality = "Right"
        except Exception as e:
            logger.warning(f"EasyOCR failed on crop: {e}")
...
async def get_metrics():
    ...
    Placeholder — full implementation in SCRUM-53 (monitoring.py).
    """
    # Import monitoring module if available, otherwise return basic stats
    try:
        from monitoring import compute_metrics
        return compute_metrics()
    except ImportError:
        # Fallback: basic stats from prediction log
        if not PREDICTIONS_LOG.exists():
            return {
                "total_predictions": 0,
                "message": "No predictions logged yet. Full metrics available after SCRUM-53.",
            }
        with open(PREDICTIONS_LOG, "r") as f:
        ...
        return {
            ...
            "model_version": MODEL_VERSION,
            "message": "Basic stats only. Full metrics available after SCRUM-53 implementation.",
        }
